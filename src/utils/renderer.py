...
def draw_skeleton(input_image, joints, draw_edges=True, vis=None, radius=None):
    ...
    colors = {
        'pink': (197/255., 27/255., 125/255.),  # L lower leg
        'light_pink': (233/255., 163/255., 201/255.),  # L upper leg
        'light_green': (161/255., 215/255., 106/255.),  # L lower arm
        'green': (77/255., 146/255., 33/255.),  # L upper arm
        'red': (215/255., 48/255., 39/255.),  # head
        'light_red': (252/255., 146/255., 114/255.),  # head
        'light_orange': (252/255., 141/255., 89/255.),  # chest
        'purple': (118/255., 42/255., 131/255.),  # R lower leg
        'light_purple': (175/255., 141/255., 195/255.),  # R upper
        'light_blue': (145/255., 191/255., 219/255.),  # R lower arm
        'blue': (69/255., 117/255., 180/255.),  # R upper arm
...
def draw_heat(input_image, input_heat):
    norm_img = np.zeros(input_heat.shape)

    input_heat = input_heat / input_heat.max()
    input_heat = input_heat * 255.

    # cv2.normalize(input_heat, norm_img, 0, 255, cv2.NORM_MINMAX)
    # print(norm_img.min(), norm_img.max())
    norm_img = np.asarray(input_heat, dtype=np.uint8)
    heat_img = cv2.applyColorMap(norm_img, cv2.COLORMAP_JET) # 注意此处的三通道热力图是cv2专有的BGR排列
    heat_img = cv2.cvtColor(heat_img, cv2.COLOR_BGR2RGB)# 将BGR图像转为RGB图像
    heat_img = heat_img.astype(np.float32)
    # img_add = cv2.addWeighted(input_image, 0.3, heat_img, 0.7, 0)
    img_add = cv2.addWeighted(input_image, 0.6, heat_img, 0.4, 0)
    return img_add
...
def visualize_reconstruction(img, img_size, gt_kp, vertices, pred_kp, camera, renderer, color='pink', focal_length=100, att_2d=None):
    ...
    #                            body_color=color)
    import colorsys
    mesh_color = {k: colorsys.hsv_to_rgb(0.5, 0.5, 1.0) for k in ['0']}
    mc = mesh_color['0']
    rend_img = renderer.render(
                    img*255.0,
                    vertices,
                    cam=[camera[0], camera[0], camera[1], camera[2]],
                    color=mc,
                    mesh_filename=None
                )
    rend_img = rend_img / 255.0
    ...
    if att_2d is not None:
        att_joint = ((att_2d + 1) * 0.5) * img_size
        att_vis = att_2d[:, 2].astype(bool)
        att_img = img.copy()
        for i in range(24):
            if att_vis[i]:
                cv2.circle(att_img, (int(att_joint[i, 0]), int(att_joint[i, 1])), radius=3, color=(1., 1., 1.))
        # att_img = draw_skeleton(img, att_joint, draw_edges=False, vis=att_vis)
        combined = np.hstack([skel_img, rend_img, att_img])
    else:
...
def visualize_reconstruction_heat(img, img_size, gt_kp, pred_kp, heatmaps):
    """Overlays gt_kp and pred_kp on img.
    Draws vert with text.
    Renderer is an instance of SMPLRenderer.
    """
    ...
    for i in range(heatmaps.shape[2]):
        heat_img = draw_heat(img_with_gt, heatmaps[:, :, i])
        heat_imgs.append(heat_img)

    combined = np.hstack(heat_imgs)
