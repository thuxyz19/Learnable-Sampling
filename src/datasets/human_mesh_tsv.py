...
class MeshTSVDataset(object):
    ...
    def __getitem__(self, idx):
        ...
        ## modify the bbox to match the settings in PARE
        # if not self.is_train:
            # scale = scale / 1.2
            # center[1] = center[1] - scale * 0.1
...
class MeshTSVYamlDataset(MeshTSVDataset):
    ...
    """
    def __init__(self, yaml_file, is_train=True, cv2_output=False, scale_factor=1, action=None):
        ...
        if action is not None:
            linelist_file = linelist_file + '.' + action
